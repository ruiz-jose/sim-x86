app.controller('Ctrl', ['$document', '$scope', '$timeout', 'cpu', 'memory', 'assembler', function ($document, $scope, $timeout, cpu, memory, assembler) {
   ...
   $scope.code = "; Sintaxis ensamblador (NASM):\n\n ;Para acceder a un contenido de la memoria se coloca entre [], por ejemplo; \n\n ;MOV AL, [x] --> copia el contenido de memoria apuntada por x al registro AL, sin embargo la instrucción MOV AL, x --> copia la dirección de memoria donde se guarda x al registro AL  \n\n;Ejemplo simple\n; x=3 , y=2, z=0\n;Operación z = y + x \n\n; sección de datos\nx: DB 3 ; Variable x=3\ny: DB 2\nz: DB 0\n\n ; sección de código\nMOV AL, [x]    ; copia contenido de [x] al registro AL\nADD AL, [y]     ; AL = AL + [y]\nMOV [z], AL     ; z <-- AL\nHLT    ; Detiene ejecución";
